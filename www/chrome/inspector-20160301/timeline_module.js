...
this.scheduleRefresh();},__proto__:WebInspector.CountersGraph.prototype};WebInspector.TimelineController=function(delegate,tracingModel)
{this._delegate=delegate;this._tracingModel=tracingModel;this._targets=[];WebInspector.targetManager.observeTargets(this);}
...
this._targets[0].tracingManager.stop();this._delegate.loadingStarted();},targetAdded:function(target)
...
this._allProfilesStoppedPromise.then(this._didStopRecordingTraceEvents.bind(this));this._allProfilesStoppedPromise=null;},_didStopRecordingTraceEvents:function()
...
{this._delegate.loadingProgress(progress);},_addCpuProfile:function(targetId,cpuProfile)
{if(!cpuProfile)
return;if(!this._cpuProfiles)
this._cpuProfiles=new Map();this._cpuProfiles.set(targetId,cpuProfile);}};WebInspector.TimelineModel=function(eventFilter)
...
startTime=endTime;}
this._inspectedTargetEvents.sort(WebInspector.TracingModel.Event.compareStartTime);this._cpuProfiles=null;this._processBrowserEvents(tracingModel);this._buildTimelineRecords();this._buildGPUTasks(tracingModel);this._insertFirstPaintEvent();this._resetProcessingState();},_processMetadataEvents:function(tracingModel,produceTraceStartedInPage)
...
this.virtualThreads().forEach(processVirtualThreadEvents.bind(this));this._records=topLevelRecords;},_buildGPUTasks:function(tracingModel)
{var mainThread=tracingModel.threadByName("GPU Process","CrGpuMain");if(!mainThread)
...
return groups.userTiming;if(!Runtime.experiments.isEnabled("timelineLatencyInfo"))
return null;if(asyncEvent.name===WebInspector.TimelineModel.RecordType.Animation)
...
this._processAnimations(animations);var range=new SegmentedRange();range.appendRange(this._drags);range.appendRange(this._cssAnimations);range.appendRange(this._scrolls);range.appendRange(this._responses);this._segments=range.segments();},_processInputLatencies:function(events)
...
this._scrolls.append(new Segment(mouseWheel.endTime,event.startTime,phases.Scroll));this._scrolls.append(this._segmentForEvent(event,phases.Scroll));mouseWheel=event;break;}}
...
{return new Segment(event.startTime,event.endTime,phase);},interactionRecords:function()
{return this._segments;},reset:function()
{var thresholdsMs=WebInspector.TimelineIRModel._mergeThresholdsMs;this._segments=[];this._drags=new SegmentedRange(merge.bind(null,thresholdsMs.mouse));this._cssAnimations=new SegmentedRange(merge.bind(null,thresholdsMs.animation));this._responses=new SegmentedRange(merge.bind(null,0));this._scrolls=new SegmentedRange(merge.bind(null,thresholdsMs.animation));function merge(threshold,first,second)
...
return(eventName.substr(prefix.length));}};;WebInspector.TimelineJSProfileProcessor={};WebInspector.TimelineJSProfileProcessor.generateTracingEventsFromCpuProfile=function(jsProfile,thread)
...
WebInspector.TimelineFlameChartEntryType={Header:Symbol("Header"),Frame:Symbol("Frame"),Event:Symbol("Event"),InteractionRecord:Symbol("InteractionRecord"),};WebInspector.TimelineFlameChartDataProvider=function(model,frameModel,irModel)
{WebInspector.TimelineFlameChartDataProviderBase.call(this,model);this._frameModel=frameModel;this._irModel=irModel;this._consoleColorGenerator=new WebInspector.FlameChart.ColorGenerator({min:30,max:55},{min:70,max:100,count:6},50,0.7);this._headerLevel1={padding:4,height:17,collapsible:true,color:"#222",font:this._font,backgroundColor:"white",nestingLevel:0,};this._headerLevel2={padding:0,height:17,collapsible:false,font:this._font,color:"#222",backgroundColor:"#bbb",nestingLevel:1,};}
...
return this._timelineData;this._timelineData=new WebInspector.FlameChart.TimelineData([],[],[],[]);this._flowEventIndexById={};this._minimumBoundary=this._model.minimumRecordTime();this._timeSpan=this._model.isEmpty()?1000:this._model.maximumRecordTime()-this._minimumBoundary;this._currentLevel=0;this._appendFrameBars(this._frameModel.frames());this._appendInteractionRecords();this._appendThreadTimelineData(WebInspector.UIString("Main Thread"),this._model.mainThreadEvents(),this._model.mainThreadAsyncEvents(),true);if(Runtime.experiments.isEnabled("gpuTimeline"))
this._appendGPUEvents();var threads=this._model.virtualThreads();var compositorThreads;var otherThreads;if(Runtime.experiments.isEnabled("timelineCollapsible")){compositorThreads=threads.filter(thread=>thread.name.startsWith("CompositorTileWorker"));otherThreads=threads.filter(thread=>!thread.name.startsWith("CompositorTileWorker"));}else{compositorThreads=[];otherThreads=threads;}
otherThreads.forEach(thread=>this._appendThreadTimelineData(thread.name,thread.events,thread.asyncEventsByGroup));if(compositorThreads.length){this._appendHeader(WebInspector.UIString("Rasterizer Threads"),this._headerLevel1);for(var i=0;i<compositorThreads.length;++i)
...
{var groups=Object.values(WebInspector.TimelineUIUtils.asyncEventGroups());for(var groupIndex=0;groupIndex<groups.length;++groupIndex){var group=groups[groupIndex];var events=asyncEvents.get(group);if(events)
this._appendAsyncEventsGroup(group.title,events);}},_appendAsyncEventsGroup:function(header,events)
...
continue;if(!groupHeaderAppended){this._appendHeader(header,this._headerLevel1);groupHeaderAppended=true;}
...
{if(!this._irModel)
return;var segments=this._irModel.interactionRecords();if(!segments||!segments.length)
return;segments.forEach(this._appendSegment,this);this._entryTypeByLevel[this._currentLevel++]=WebInspector.TimelineFlameChartEntryType.InteractionRecord;},_appendFrameBars:function(frames)
...
return"white";if(type===WebInspector.TimelineFlameChartEntryType.Header)
return"#aaa";if(type===WebInspector.TimelineFlameChartEntryType.InteractionRecord)
return WebInspector.TimelineUIUtils.interactionPhaseColor((this._entryData[entryIndex]));return"";},decorateEntry:function(entryIndex,context,text,barX,barY,barWidth,barHeight)
{var data=this._entryData[entryIndex];var type=this._entryType(entryIndex);if(type===WebInspector.TimelineFlameChartEntryType.Frame){var vPadding=1;var hPadding=1;var frame=(data);barX+=hPadding;barWidth-=2*hPadding;barY+=vPadding;barHeight-=2*vPadding+1;context.fillStyle=frame.idle?"white":"#eee";context.fillRect(barX,barY,barWidth,barHeight);if(frame.hasWarnings())
...
this._timelineData.groups.push({startLevel:this._currentLevel,name:title,expanded:expanded,style:style});else
this._appendHeaderRecord(title)},_appendHeaderRecord:function(title)
{var index=this._entryData.length;this._entryIndexToTitle[index]=title;this._entryData.push(null);this._entryTypeByLevel[this._currentLevel]=WebInspector.TimelineFlameChartEntryType.Header;this._timelineData.entryLevels[index]=this._currentLevel++;this._timelineData.entryTotalTimes[index]=this._timeSpan;this._timelineData.entryStartTimes[index]=this._minimumBoundary;},_appendEvent:function(event,level)
...
{var frame=WebInspector.TimelineUIUtils.topStackFrame(event);return frame?linkifyLocationAsText(frame.scriptId,frame.lineNumber,frame.columnNumber):null;}}
...
this._irModel=new WebInspector.TimelineIRModel();this._controller=new WebInspector.TimelineController(this,this._tracingModel);if(Runtime.experiments.isEnabled("cpuThrottling"))
...
this._setState(WebInspector.TimelinePanel.State.StopPending);this._autoRecordGeneration=null;this._controller.stopRecording();this._setUIControlsEnabled(true);},_onSuspendStateChanged:function()
...
{this._tracingModel.reset();this._model.reset();this._showRecordingHelpMessage();this.requestWindowTimes(0,Infinity);delete this._selection;this._frameModel.reset();this._overviewPane.reset();for(var i=0;i<this._currentViews.length;++i)
...
if(this._statusPane)
this._statusPane.updateStatus(WebInspector.UIString("Processing timeline\u2026"));this._model.setEvents(this._tracingModel,loadedFromFile);this._frameModel.reset();this._frameModel.addTraceEvents(this._model.target(),this._model.inspectedTargetEvents(),this._model.sessionId()||"");if(this._irModel)
this._irModel.populate(this._model);if(this._statusPane)
