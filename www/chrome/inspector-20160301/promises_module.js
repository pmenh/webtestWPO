...
this.parentId=details.parentId;if(details.creationTime)
this.creationTime=details.creationTime;if(details.settlementTime)
this.settlementTime=details.settlementTime;if(details.creationStack){this.creationStack=details.creationStack;if(this.creationStack.callFrames.length)
...
{var node=this._dataGrid.dataGridNodeFromNode(anchor);var details=this._promiseDetailsByDebuggerModel.get(this._debuggerModel).get(node.promiseId());var stackTrace;if(anchor.classList.contains("created-column"))
stackTrace=details.creationStack;else
stackTrace=details.settlementStack;var content=WebInspector.DOMPresentationUtils.buildStackTracePreviewContents(this._debuggerModel.target(),this._linkifier,stackTrace);popover.setCanShrink(true);popover.showForAnchor(content,anchor);},__proto__:WebInspector.VBox.prototype}
