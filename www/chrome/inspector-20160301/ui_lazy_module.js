...
{return true;},selectItemWithQuery:function(itemIndex,promptValue)
...
WebInspector.CommandMenuDelegate.prototype={_appendAvailableCommands:function()
...
{return left.title().compareTo(right.title());}},itemCount:function()
...
{return 1;},renderItem:function(itemIndex,query,titleElement,subtitleElement)
{var command=this._commands[itemIndex];titleElement.textContent=command.title();this.highlightRanges(titleElement,query);subtitleElement.textContent=command.shortcut();},selectItem:function(itemIndex,promptValue)
...
{this._title=title;this._key=title+"\0"+key;this._shortcut=shortcut;this._executeHandler=executeHandler;this._availableHandler=availableHandler;}
WebInspector.CommandMenu.Command.prototype={title:function()
...
{var key=keys?keys.split(",").join("\0"):"";title=category?WebInspector.UIString("%s: %s",category,title):title;return new WebInspector.CommandMenu.Command(title,key,shortcut,executeHandler,availableHandler);}
...
{new WebInspector.FilteredListWidget(new WebInspector.CommandMenuDelegate(),false).showAsDialog();InspectorFrontendHost.bringToFront();return true;}};WebInspector.FlameChartDelegate=function(){}
...
{var hash=String.hashCode(id);var h=this._indexToDistinctValueInSpace(this._colors.size,this._hueSpace);var s=this._indexToValueInSpace(hash,this._satSpace);var l=this._indexToValueInSpace(hash,this._lightnessSpace);var a=this._indexToValueInSpace(hash,this._alphaSpace);return"hsla("+h+", "+s+"%, "+l+"%, "+a+")";},_indexToValueInSpace:function(index,space)
{if(typeof space==="number")
return space;index%=space.count;return space.min+Math.floor(index/(space.count-1)*(space.max-space.min));},_indexToDistinctValueInSpace:function(index,space)
{if(typeof space==="number")
return space;index|=0;var result=0;var count=0;do{result=(result<<1)|(index&1);index>>=1;++count;}while(index);result/=1<<count;return space.min+Math.floor(result*(space.max-space.min));}}
...
return;var groupIndex=this._coordinatesToGroupIndex(event.offsetX,event.offsetY);if(groupIndex>=0){var group=this._rawTimelineData.groups[groupIndex];if(!group.style.collapsible)
return;group.expanded=!group.expanded;this._groupExpansionState[group.name]=group.expanded;if(this._groupExpansionSetting)
this._groupExpansionSetting.set(this._groupExpansionState);this._updateLevelPositions();this._updateHeight();this._resetCanvas();this._draw(this._offsetWidth,this._offsetHeight);return;}
this._hideRangeSelection();this.dispatchEventToListeners(WebInspector.FlameChart.Events.EntrySelected,this._highlightedEntryIndex);},_onMouseWheel:function(e)
{if(!this._enabled())
...
return-1;var cursorTime=this._cursorTime(x);var cursorLevel=this._visibleLevelOffsets.upperBound(y)-1;if(cursorLevel<0)
...
{if(x<0||y<0)
return-1;y+=this._scrollTop;var groups=this._rawTimelineData.groups||[];var group=this._groupOffsets.upperBound(y)-1;if(group>=0&&y-this._groupOffsets[group]<groups[group].style.height)
return group;return-1;},_markerIndexAtPosition:function(x)
...
{var context=this._canvas.getContext("2d");var top=this._scrollTop;var ratio=window.devicePixelRatio;var barHeight=this._barHeight;var textBaseHeight=barHeight-this._dataProvider.textBaseline();var groups=this._rawTimelineData.groups||[];var groupOffsets=this._groupOffsets;var lastGroupOffset=Array.prototype.peekLast.call(groupOffsets);var firstVisibleGroup=Math.max(groupOffsets.upperBound(top)-1,0);context.save();context.scale(ratio,ratio);context.translate(0,-top);context.fillStyle="#eee";forEachGroup((offset,index,group)=>{var paddingHeight=group.style.padding;if(paddingHeight<5)
...
context.strokeStyle="#ddd";context.beginPath();forEachGroup((offset,index,group,isFirst)=>{if(isFirst||group.style.padding<4)
return;hLine(offset-group.style.padding+1.5);hLine(offset-1.5);});hLine(lastGroupOffset+1.5);context.stroke();context.strokeStyle="#bbb";context.beginPath();forEachGroup((offset,index,group,isFirst)=>{if(isFirst||group.style.padding<4)
return;hLine(offset-group.style.padding+0.5);hLine(offset-0.5);});hLine(lastGroupOffset+0.5);context.stroke();forEachGroup((offset,index,group)=>{if(!group.style.collapsible||group.expanded){context.fillStyle=group.style.backgroundColor;context.fillRect(0,offset,width,group.style.height);return;}
var nextGroup=index+1;while(nextGroup<groups.length&&groups[nextGroup].style.nestingLevel>group.style.nestingLevel)
nextGroup++;var endLevel=nextGroup<groups.length?groups[nextGroup].startLevel:this._dataProvider.maxStackDepth();this._drawCollapsedOverviewForGroup(offset+1,group.startLevel,endLevel);});var headerLeftPadding=6;var arrowSide=8;var expansionArrowX=headerLeftPadding+arrowSide/2;context.font="11px "+WebInspector.fontFamily();context.save();context.fillStyle="rgba(255, 255, 255, 0.5)";forEachGroup(drawBackground.bind(this));context.restore();context.fillStyle="#6e6e6e";context.beginPath();forEachGroup((offset,index,group)=>{if(group.style.collapsible)
drawExpansionArrow(expansionArrowX,offset+textBaseHeight-arrowSide/2,!!group.expanded)});context.fill();forEachGroup((offset,index,group)=>{context.fillStyle=group.style.color;context.font=group.style.font;context.fillText(group.name,Math.floor(expansionArrowX+arrowSide),offset+textBaseHeight);});context.strokeStyle="#ddd";context.beginPath();forEachGroup((offset,index,group)=>{if(group.expanded)
hLine(offset+barHeight-0.5);});context.stroke();context.restore();function hLine(y)
...
{var levelCount=this._timelineLevels.length;var groups=this._rawTimelineData.groups||[];this._visibleLevelOffsets=new Uint32Array(levelCount);this._visibleLevels=new Uint8Array(levelCount);this._groupOffsets=new Uint32Array(groups.length+1);var groupIndex=-1;var currentOffset=WebInspector.FlameChart.DividersBarHeight;var visible=true;var groupStack=[{nestingLevel:-1,visible:true}];for(var level=0;level<levelCount;++level){while(groupIndex<groups.length-1&&level===groups[groupIndex+1].startLevel){++groupIndex;var style=groups[groupIndex].style;var nextLevel=groupStack.peekLast().nestingLevel<style.nestingLevel;if(!nextLevel)
groupStack.pop();var thisGroupIsVisible=style.collapsible?groups[groupIndex].expanded:true;var parentGroupIsVisible=groupStack.peekLast().visible;visible=thisGroupIsVisible&&parentGroupIsVisible;groupStack.push({nestingLevel:style.nestingLevel,visible:visible});if(parentGroupIsVisible)
currentOffset+=nextLevel?0:style.padding;this._groupOffsets[groupIndex]=currentOffset;if(parentGroupIsVisible)
currentOffset+=style.height;}
this._visibleLevels[level]=visible;this._visibleLevelOffsets[level]=currentOffset;if(visible)
currentOffset+=this._barHeight;}
if(groupIndex>=0)
this._groupOffsets[groupIndex+1]=currentOffset;},setSelectedEntry:function(entryIndex)
